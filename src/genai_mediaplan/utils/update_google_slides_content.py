...
DEFINITION_SLIDE_INDEX = 1
CHART_SLIDE_INDEX = 7
PERSONA_SLIDE_INDEX_4 = 9
PERSONA_SLIDE_INDEX_6 = 10
GEO_SLIDE_INDEX = 11
...
def get_content_to_replace_in_slides(ppt_title, llm_response_json, audience_forecast):
...
def delete_slides_requests(presentation_id, persona_slide_index):
    requests = []
    slide_indexes_to_delete = [persona_slide_index]
    presentation = slides_service.presentations().get(presentationId=presentation_id).execute()
...
def get_copy_of_presentation(ppt_title, llm_response_json, audience_forecast, demographic_data):
    copied_file = drive_service.files().copy(
        ...
        body={
            'name': f'TIL_CohortDashboard_{"".join(ppt_title.split(" "))}_Forecast_{datetime.now().strftime("%B%Y")}',
    ...
    sheet_copy = drive_service.files().copy(
        ...
        body={
            'name': f'Charts_{"".join(ppt_title.split(" "))}_{datetime.now().strftime("%B%Y")}',
            'parents': [SHARED_FOLDER_ID]
        }
    ).execute()
    ...
    data, persona_data = get_content_to_replace_in_slides(ppt_title, llm_response_json, audience_forecast)
    ...
    update_requests_definition, delete_requests_definition = update_definition_reach(slides_service, copied_file_id, DEFINITION_SLIDE_INDEX, audience_forecast, llm_response_json.get("cohort_definition", " "))
