...
class GenaiMediaplan():
    ...
    @agent
    def title_agent(self) -> Agent:
        return Agent(
            config=self.agents_config['title_agent'],  # type: ignore[index]
            verbose=True
        )
    ...
    @task
    def title_task(self) -> Task:
        return Task(
            config=self.tasks_config['title_task'], # type: ignore[index]
        )
